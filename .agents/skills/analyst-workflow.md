...
5. **Widen.** Add dimensions or extend the time range only after the narrow version is stable. Re-`premortem` if widening crosses the join/scan thresholds in rules.md #9.
6. **Crystallise.** If the question will be asked again, hand off to `bi-view` (analytical layer) or `ops-dataset` (action queue). If it's a one-off, capture a regression golden for the headline number (`regression-test`) so the answer is reproducible later.
