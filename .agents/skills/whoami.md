---
name: whoami
description: Resolve which Root org the current AWS credentials and ROOT_ORG_ID point at, by querying the organisations table. Use at session start, whenever ROOT_ORG_ID may have changed, or whenever the user asks "which org am I in?". Do NOT use if you've already called it this session and ROOT_ORG_ID hasn't changed.
---

# Skill: whoami

Identifies the active organisation. The four printed lines (`Org`, `Org ID`, `Region`, `Env`) are the anchor every later answer in the session refers back to — print them once at session start.
...
The script runs `SELECT id, name FROM organisations WHERE id = '$ROOT_ORG_ID'`. The org id is the same string used as the Athena workgroup, the schema/database, and the S3 results prefix.
