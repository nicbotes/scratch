...
2. **Cluster notes by `target`.** Use the session-local feedback log and the session trace together — the trace tells you which tools ran and what they scanned; the feedback tells you which moments hurt.
...
5. **Hand back a one-screen summary:** top 3 proposals (by note count or by judgment of impact), total entries reviewed, files touched. The user applies the patches with normal review.
6. **Never edit live files.** Rules.md #13. If a proposed change feels obvious enough to apply immediately, that's a signal to ask the user — not to bypass review.
...
→ Cross-references: `rules.md` #13 (no live edits), #14 (red golden = stop-the-line).
