#!/usr/bin/env bash
# whoami.sh
# Resolve the active org by querying the organizations table
...
sql="SELECT organization_id, name FROM organizations WHERE organization_id = '$ROOT_ORG_ID' LIMIT 1"
qid="$(run_athena "$sql")"
...
if [[ -z "$row" ]]; then
  echo "error: no row in organizations for organization_id=$ROOT_ORG_ID" >&2
