...
# 16 hex chars. Same value -> same hash within the salt scope (joins
# preserved). Different salt scopes (e.g. different orgs) produce different
# hashes so cross-org joins don't accidentally line up.
#
# Default salt is hash_id($ROOT_ORG_ID) — the same helper Phase 6 uses for
# identifier obfuscation in regression goldens. Same algorithm, two purposes
# (metadata identifiers vs data values).
#
...
# <col>_hash so the consumer knows the column has been pseudonymised.
#
# Use case: between athena-query.sh --to-file and duckdb-query.sh, when the
# agent needs to slice PII-bearing data without ever seeing raw values.
...
# Default salt: org-hash (same value -> same hash within an org)
if [[ -z "$salt" ]]; then
  if [[ -n "${ROOT_ORG_ID:-}" ]]; then
    salt="$(hash_id "$ROOT_ORG_ID")"
  else
    echo "error: no ROOT_ORG_ID set and no --salt provided. Per-org salting is the default; either set ROOT_ORG_ID or pass --salt <key> explicitly." >&2
    exit 64
  fi
fi
...
# Verify all requested columns existed
present = set(header)
